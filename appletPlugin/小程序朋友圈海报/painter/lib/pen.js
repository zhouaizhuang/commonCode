import { getApp } from './plugin-compat.js'
...
export default class Painter {
  ...
  _doClipTop(borderRadius, width, height) {
    if (borderRadius && width && height) {
      let r = Math.min(borderRadius.toPx(), width / 2, height / 2);
      // 防止在某些机型上周边有黑框现象，此处如果直接设置 setFillStyle 为透明，在 Android 机型上会导致被裁减的图片也变为透明， iOS 和 IDE 上不会
      // setGlobalAlpha 在 1.9.90 起支持，低版本下无效，但把 setFillStyle 设为了 white，相对默认的 black 要好点
      this.ctx.setGlobalAlpha(0);
      this.ctx.setFillStyle('white');
  ...
  _doClip24(borderRadius,width,height){
    if (borderRadius && width && height) {
      let r = 0,_r = 0;
      _r = r = Math.min(borderRadius.toPx(), width / 2, height / 2);
      // 防止在某些机型上周边有黑框现象，此处如果直接设置 setFillStyle 为透明，在 Android 机型上会导致被裁减的图片也变为透明， iOS 和 IDE 上不会
      // setGlobalAlpha 在 1.9.90 起支持，低版本下无效，但把 setFillStyle 设为了 white，相对默认的 black 要好点
      this.ctx.setGlobalAlpha(0);
      this.ctx.setFillStyle('white');
  ...
  _doClipRadius(rs,width,height){
    if (rs && width && height) {
      ...
      this.ctx.moveTo(r1 - width / 2,-height / 2);
      this.ctx.arcTo(width / 2,-height / 2,width / 2,height / 2,r2);
      this.ctx.arcTo(width / 2,height / 2,-width / 2,height / 2,r3);
      this.ctx.arcTo(-width / 2,height / 2,-width / 2,-height / 2,r4);
      this.ctx.arcTo(-width / 2,-height / 2,width / 2,-height / 2,r1);
      this.ctx.closePath();
      this.ctx.fill();
      // 在 ios 的 6.6.6 版本上 clip 有 bug，禁掉此类型上的 clip，也就意味着，在此版本微信的 ios 设备下无法使用 border 属性
      if (!(getApp().systemInfo &&
  ...
  _doBorder(view, width, height) {
    ...
    if (GD.api.isGradient(view.css.borderColor)) {
      let grad = GD.api.getGradient(view.css.borderColor, width, height, this.ctx);
  ...
  _preProcess(view, notClip) {
    ...
    if(!notClip && view.css && view.css.topRadius){
      this._doClipTop(view.css.topRadius, width, height);
    }
    if(!notClip && view.css && view.css.radius24){
      this._doClip24(view.css.radius24, width, height);
    }
    if(!notClip && view.css && view.css.rs){
      this._doClipRs(view.css.rs, width, height);
    }
    if(!notClip && view.css && view.css.radius){
      this._doClipRadius(view.css.radius, width, height);
    }
    if(!notClip && view.css && view.css.clipPoints){
      this._doClipPoints(view.css.clipPoints,width,height);
    }
    this._doShadow(view);
  ...
  _fillAbsText(view) {
    ...
    if (GD.api.isGradient(view.css.color)) {
      let grad = GD.api.getGradient(view.css.color, width, height, this.ctx);
      this.ctx.fillStyle = grad;
    } else {
      this.ctx.setFillStyle(view.css.color || 'black');
    }

    if(view.css.strokeStyle){
      if(GD.api.isGradient(view.css.strokeStyle)){
        let strokeGD = GD.api.getGradient(view.css.strokeStyle, width, height, this.ctx);
        this.ctx.strokeStyle = strokeGD;
        this.ctx.lineWidth = view.css.strokeWidth.toPx();
      }else{
        this.ctx.strokeStyle = view.css.strokeStyle;
      }
    }
    ...
    for (let i = 0; i < lines; ++i) {
      ...
      if (view.css.textStyle === 'stroke' || view.css.strokeStyle) {
  ...
  _fillAbsTextLines(view) {
    ...
    let lines = view.position.ext.lines;
    let _maxLines = view.css.maxLines;
    let maxLines = lines.length;
    const lineHeight = parseInt(view.css.lineHeight);
    const measuredWith = width;

    if(_maxLines){
      let text = lines[_maxLines - 1];
      if(maxLines >= _maxLines){
        ...
        text += '...';
        lines[_maxLines - 1] = text;
      }
      maxLines = Math.min(maxLines,_maxLines);
    }
